-- ============================================================
-- 109: 영수증 박제 양식 정정 (사장 모델 반영)
--
-- 사장 양식 (회의 2026-05-03):
--   전잔  : 기존외상
--   당일  : 판매금액 (= total_amount)
--   결제  : 판매금액 (결제수단 라벨 표시)
--   당잔  : 결제수단 = credit → 기존외상 + 판매금액 (외상 누적)
--           결제수단 = cash/transfer → 기존외상 (즉시 입금, 외상 변동 X)
--   derived 주문 (미송/보류 출고): 외상 변동 = revenue (보류분만, 미송 0)
--                                  당잔 = 전잔 + revenue
--
-- 변경: issue_receipt_snapshot 시그니처 (UUID, NUMERIC, NUMERIC) → (UUID, NUMERIC)
--       호출자는 prev_balance 만 전달. post_balance 는 함수가 결제수단 보고 자동 계산.
-- ============================================================

-- ── issue_receipt_snapshot 재정의 (2인자) ─────────────────
DROP FUNCTION IF EXISTS issue_receipt_snapshot(UUID, NUMERIC, NUMERIC);
...
BEGIN
  ...
  IF v_order.derived_from_order_id IS NOT NULL THEN
    -- derived 주문: 외상 변동 = revenue (미송 0 / 보류 출고분)
  ...
  ELSE
    -- credit (외상) → 누적
    v_post_balance := p_prev_balance + v_order.total_amount;
  END IF;
...
GRANT EXECUTE ON FUNCTION issue_receipt_snapshot(UUID, NUMERIC) TO authenticated;


-- ── process_pending_ship 재정의 (2인자 호출) ──────────────
...
BEGIN
  ...
  -- 영수증 prev_balance 시작 시점 외상 snapshot
  ...
  -- 5) 영수증 박제 (109: post 는 함수 안에서 결제수단/derived 분기 자동 계산)
...
-- ── refresh_order_revenue 재정의 (2인자 호출) ──────────────
...
BEGIN
  ...
  IF v_is_processed AND NOT v_prev_processed AND v_revenue > 0 THEN
    ...
    -- 영수증 박제 (109: 2인자, post 는 함수가 결제수단 자동 계산)
