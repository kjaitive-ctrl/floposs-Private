...
// 발송/미발송내역 미리보기 + 출력 + 카톡 복사 모달.
// 두 모드:
//   - { kind: "customer", customerId, session: "today"|"all", which: "backorder"|"hold"|"all" }
//   - { kind: "product",  productId }
//
// ReceiptPrintModal 과 동일 패턴 — 영수증 프린터 + 카톡 복사 동일.
...
export type PendingMode =
  | { kind: "customer"; customerId: string; session: "today" | "all"; which?: "backorder" | "hold" | "all" }
...
export function PendingPrintModal({ mode, onClose, onPrinted }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!authSession) { setLoadError("세션 없음"); return; }

      const url = mode.kind === "customer"
        ? `/api/pending/customer/${mode.customerId}/print?session=${mode.session}&kind=${mode.which ?? "all"}`
        : `/api/pending/product/${mode.productId}/print`;

      const res = await fetch(url, { headers: { Authorization: `Bearer ${authSession.access_token}` } });
      if (cancelled) return;
      if (!res.ok) {
  ...
  return (
    <Modal onClose={onClose} size="md">
      <div className="p-5">
        <h2 className="text-base font-bold text-gray-900 mb-3">발송/미발송내역 미리보기</h2>
