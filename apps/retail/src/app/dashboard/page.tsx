...
// ── 날씨 (Open-Meteo 11일, 상업런칭 전 KMA 전환) ──
...
// ── 오늘 할일 (업무루틴, 2주 사이클) ──
function TodayTasksWidget({ tenantId }: { tenantId: string }) {
      ...
      {loading ? <div className="text-xs text-gray-400 px-4 py-6 text-center">불러오는 중…</div>
        : tasks.length === 0 ? (
          <div className="text-xs text-gray-400 px-4 py-6 text-center">
            오늘 할일이 없어요. <Link href="/routines" className="text-primary hover:underline">업무루틴 세팅</Link>
          </div>
        ) : (
          <ul className="divide-y divide-gray-50">
            {tasks.map((r) => {
...
// ── 다가오는 일정 (오늘~+14일) ──
function UpcomingScheduleWidget({ tenantId }: { tenantId: string }) {
